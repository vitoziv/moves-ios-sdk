...
+ (BOOL)validateValuesFrom:(id)json
    ...
    if(!json) {
        [MVRPJSONValidator log:@"RPJSONValidator Error: json parameter is nil -- there is nothing to validate. Returning NO"];
        if(error) {
    ...
    if(![json isKindOfClass:[NSDictionary class]] && ![json isKindOfClass:[NSArray class]]) {
        [MVRPJSONValidator log:@"RPJSONValidator Error: json parameter is not valid JSON (it is not an NSArray or NSDictionary). Returning NO"];
        if(error) {
            *error = [NSError errorWithDomain:RPJSONValidatorErrorDomain
                                         code:RPJSONValidatorErrorBadJSONParameter
                                     userInfo:@{
    ...
    if(!requirements) {
        [MVRPJSONValidator log:@"RPJSONValidator Error: requirements parameter is nil -- there are no requirements. Returning NO"];
        if(error) {
            *error = [NSError errorWithDomain:RPJSONValidatorErrorDomain
    ...
    if(![requirements isKindOfClass:[NSDictionary class]]) {
        [MVRPJSONValidator log:@"RPJSONValidator Error: requirements parameter is not an NSDictionary. Returning NO"];
        if(error) {
            *error = [NSError errorWithDomain:RPJSONValidatorErrorDomain
                                         code:RPJSONValidatorErrorBadRequirementsParameter
                                     userInfo:@{
    ...
    return [MVRPJSONValidator validateValuesFrom:json
...
+ (BOOL)validateValuesFrom:(id)json
    ...
    [requirements enumerateKeysAndObjectsUsingBlock:^(id requirementsKey, id requirementsValue, BOOL *stop) {
        ...
        if([requirementsValue isKindOfClass:[MVRPValidatorPredicate class]] && [requirementsKey isKindOfClass:[NSString class]]) {
            [(MVRPValidatorPredicate *)requirementsValue validateValue:jsonValue withKey:requirementsKey];
            
            if([[(MVRPValidatorPredicate *)requirementsValue failedRequirementDescriptions] count]) {
                ...
                [failingKeys setObject:[(MVRPValidatorPredicate *)requirementsValue failedRequirementDescriptions] forKey:requirementsKey];
